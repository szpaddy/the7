		...
		foreach ( $buttons as $button ) {
			...
			switch( $button ) {
				case 'tsina':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'tsina';
					$share_title = _x('分享到新浪微博', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=tsina&' );
					break;
				case 'renren':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'renren';
					$share_title = _x('分享到人人网', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=renren&' );
					break;
				case 'qzone':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'qzone';
					$share_title = _x('分享到QQ空间', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=qzone&' );
					break;
				case 'tqq':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'tqq';
					$share_title = _x('分享到腾讯微博', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=tqq&' );
					break;
				case 'kaixin001':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'kaixin001';
					$share_title = _x('分享到开心网', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=kaixin001&' );
					break;
				case 'weixin':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'weixin';
					$share_title = _x('分享到微信', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=weixin&' );
					break;
				case 'douban':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'douban';
					$share_title = _x('分享到豆瓣', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=douban&' );
					break;
				case 'taobao':
                    $t = str_replace(' ', '+', $t);
					$classes[] = 'taobao';
					$share_title = _x('分享到淘江湖', 'share buttons', LANGUAGE_ZONE);
					$url = add_query_arg( array('url' => $u, 'title' => $t), $protocol . '://www.jiathis.com/send/?webid=taobao&' );
					break;
				case 'twitter':
...
if ( ! function_exists( 'presscore_get_the_clear_content' ) ) :
	...
	function presscore_get_the_clear_content() {
		$djexcerpt = of_get_option('general-rel_posts_djexcerpt');
		$content = get_the_content( '' );
		$content = strip_shortcodes( $content );
		$content = apply_filters( 'the_content', $content );
		$content = str_replace( ']]>', ']]&gt;', $content );
        $content = strip_tags($content);
		$content = mb_substr($content,0,$djexcerpt,'utf-8').'...<a href="'.get_permalink($post->ID).'" class="details more-link" rel="nofollow">详情</a>';
		return $content;
	}
